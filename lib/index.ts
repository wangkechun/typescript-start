...
export function run() {
  var nowPath = '.';
  var definitelyTypedPath = path.resolve(path.join(__dirname, '../DefinitelyTyped/'));
  ...
  var allDOtDDOTTsContent = '//generated by typescript-start, npm -g install typescript-start to use it\n';
  for (var i in DotDDotTsFiles) {
    var t = DotDDotTsFiles[i];
    allDOtDDOTTsContent += `/// <reference path="./${t}" />\n`;
  }
  console.log(allDOtDDOTTsContent);
  fse.outputFileSync(path.join(nowPath, 'typings', 'all.d.ts'), allDOtDDOTTsContent);
